...
func TestDocs_Search_RanksTitleAndTagMatchesHighest(t *testing.T) {
	fsys := testFS()
	fsys["kb/guides/routing/passing.md"] = &fstest.MapFile{Data: []byte(`---
...
func TestDocs_Search_BoundsSnippetWidth(t *testing.T) {
	fsys := testFS()
	fsys["kb/guides/routing/wide.md"] = &fstest.MapFile{Data: []byte(
		"---\ntitle: Wide\nsummary: Wide.\ncategory: guides\n---\n\n# Wide\n\nneedle " +
...
func TestDocs_Search_MergesAdjacentHits(t *testing.T) {
	fsys := testFS()
	fsys["kb/guides/routing/dense.md"] = &fstest.MapFile{Data: []byte(`---
title: Dense
summary: Dense.
category: guides
---
