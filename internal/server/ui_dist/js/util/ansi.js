// ANSI escape sequence → HTML renderer. Ported from lib/ansi.ts. SGR
// (select graphic rendition) sequences become inline-styled spans; other CSI
// sequences are dropped. Text content is HTML-escaped.
import { escapeHtml } from "../dom.js";

const DEFAULT_STYLE = { bold: false, italic: false, underline: false, strikethrough: false, fg: null, bg: null };
...
function applySgr(prev, params, palette) {
  let style = prev;
  const update = (patch) => {
    if (style === prev) {
      style = { ...prev };
 ...
 */
export function ansiToHtml(text, dark = false) {
