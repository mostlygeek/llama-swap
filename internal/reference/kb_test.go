...
func TestDocs_KB_ParsesFrontmatter(t *testing.T) {
	...
	if got.Source != "internal/docagent/db/guides/routing/first-tip.md" {
...
func TestDocs_KB_SkipsReadme(t *testing.T) {
	fsys := testFS()
	fsys["internal/docagent/db/guides/routing/README.md"] = &fstest.MapFile{Data: []byte("# nested guide index\n")}
	d := New(fsys)
	for _, id := range []string{"guides/README", "guides/routing/README", "internal/docagent/db/README"} {
...
func TestDocs_KB_ToleratesBadFrontmatter(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			fsys := testFS()
			fsys["internal/docagent/db/guides/routing/odd.md"] = &fstest.MapFile{Data: []byte(tt.body)}
...
func TestDocs_KB_SkipsNonMarkdown(t *testing.T) {
	fsys := testFS()
	fsys["internal/docagent/db/guides/routing/notes.txt"] = &fstest.MapFile{Data: []byte("not markdown")}
	fsys["internal/docagent/db/guides/routing/diagram.png"] = &fstest.MapFile{Data: []byte("binary")}
...
func TestDocs_KB_EmptyArticleIsNotIndexed(t *testing.T) {
	fsys := testFS()
	fsys["internal/docagent/db/guides/routing/empty.md"] = &fstest.MapFile{Data: []byte("---\ntitle: Nothing\n---\n\n")}
