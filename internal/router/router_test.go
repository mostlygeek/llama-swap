...
func TestExtractAndSetModel_JSON(t *testing.T) {
	body := `{"model":"llama3"}`
	r, _ := http.NewRequest(http.MethodPost, "/v1/chat/completions", strings.NewReader(body))
	r.Header.Set("Content-Type", "application/json")

	if err := ExtractAndSetModel(r); err != nil {
		t.Fatalf("ExtractAndSetModel: %v", err)
	}

	model, ok := GetModel(r.Context())
	if !ok {
		t.Fatalf("model not present in request context after ExtractAndSetModel")
	}
	if model != "llama3" {
		t.Errorf("want %q got %q", "llama3", model)
	}
...
func TestExtractAndSetModel_GET(t *testing.T) {
...
func TestExtractAndSetModel_ExtractError(t *testing.T) {
	r, _ := http.NewRequest(http.MethodGet, "/", nil)

	if err := ExtractAndSetModel(r); err == nil {
		t.Fatalf("expected error from ExtractAndSetModel when model missing")
	}

	if _, ok := GetModel(r.Context()); ok {
		t.Errorf("model unexpectedly set on context after failed ExtractAndSetModel")
	}
}
