...
EXPECTED_BINARIES=(llama-server llama-cli llama-bench whisper-server whisper-cli sd-server sd-cli llama-swap vllm-wrapper)
if [[ "$BACKEND" == "cuda" ]]; then
...
VERIFIED_LIST="llama-server, llama-cli, llama-bench, whisper-server, whisper-cli, sd-server, sd-cli, llama-swap, vllm-wrapper"
if [[ "$BACKEND" == "cuda" ]]; then
