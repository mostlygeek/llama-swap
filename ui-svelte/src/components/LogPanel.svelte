<script lang="ts">
  ...
  // Create persistent stores for this panel
  ...
  let fontSize = $state<"xxs" | "xs" | "small" | "normal">("normal");
  let wrapText = $state(false);
  let showFilter = $state(false);

  // Subscribe to persistent stores
  onMount(() => {
    const unsubFontSize = fontSizeStore.subscribe((v) => (fontSize = v));
    const unsubWrapText = wrapTextStore.subscribe((v) => (wrapText = v));
    const unsubShowFilter = showFilterStore.subscribe((v) => (showFilter = v));
  ...
  let textWrapClass = $derived(wrapText ? "whitespace-pre-wrap" : "whitespace-pre");
  ...
  <div class="p-4">
    <div class="flex items-center justify-between">
      ...
      <div class="flex gap-2 items-center">
        <button class="btn border-0" onclick={toggleFontSize}>
          <!-- Font size icon -->
        ...
        <button class="btn border-0" onclick={toggleWrapText}>
          {#if wrapText}
            <!-- Text wrap icon -->
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-4 h-4">
              <path
          ...
          {:else}
            ...
            <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-4 h-4">
              <path
                fill-rule="evenodd"
                d="M3 6.75A.75.75 0 0 1 3.75 6h16.5a.75.75 0 0 1 0 1.5H3.75A.75.75 0 0 1 3 6.75ZM3 12a.75.75 0 0 1 .75-.75h10.5a.75.75 0 0 1 0 1.5H3.75A.75.75 0 0 1 3 12Zm0 5.25a.75.75 0 0 1 .75-.75h16.5a.75.75 0 0 1 0 1.5H3.75a.75.75 0 0 1-.75-.75Z"
                clip-rule="evenodd"
              />
            </svg>
          {/if}
    ...
    {#if showFilter}
      <div class="mt-2 w-full">
        <div class="flex gap-2 items-center w-full">
          ...
          />
          <button class="pl-2" onclick={() => (filterRegex = "")}>
            <!-- Close circle icon -->
