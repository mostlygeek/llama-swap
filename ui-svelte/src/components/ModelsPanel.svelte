<script lang="ts">
  ...
  let showUnlisted = $state(true);
  let showIdorName = $state<"id" | "name">("id");

  onMount(() => {
    const unsubUnlisted = showUnlistedStore.subscribe((v) => (showUnlisted = v));
    const unsubIdorName = showIdorNameStore.subscribe((v) => (showIdorName = v));
...
<div class="card h-full flex flex-col">
  <div class="shrink-0">
    <div class="flex justify-between items-baseline">
      ...
      {#if $isNarrow}
        <div class="relative">
          <button class="btn text-base flex items-center gap-2 py-1" onclick={() => (menuOpen = !menuOpen)}>
            <!-- Menu icon -->
          ...
          {#if menuOpen}
            <div class="absolute right-0 mt-2 w-48 bg-surface border border-gray-200 dark:border-white/10 rounded shadow-lg z-20">
              ...
              >
                ...
                {showIdorName === "id" ? "Show Name" : "Show ID"}
              </button>
              <button
                class="w-full text-left px-4 py-2 hover:bg-secondary-hover flex items-center gap-2"
                onclick={() => {
          ...
          <tr class="border-b hover:bg-secondary-hover border-gray-200">
            <td class={model.unlisted ? "text-txtsecondary" : ""}>
              ...
              {#if model.description}
                <p class={model.unlisted ? "text-opacity-70" : ""}>
