#!/bin/bash
# Install stable-diffusion.cpp - clone, build with CUDA, and install binaries
...
# Clone and checkout (with submodules)
echo "=== Cloning stable-diffusion.cpp at ${COMMIT_HASH} ==="
git clone --filter=blob:none --no-checkout https://github.com/leejet/stable-diffusion.cpp.git /src/stable-diffusion.cpp
