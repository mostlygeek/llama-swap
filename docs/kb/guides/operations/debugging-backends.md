...
| `starting`, then `running Xm, still not ready (probe failing?)` after the startup window (default 10m) | server bound to 127.0.0.1 (missing `--host 0.0.0.0` / `--listen-ip 0.0.0.0` — the engine's log says `listening on http://127.0.0.1:…`), wrong `--health-path`, or a load that outlasted the startup window | `starting` inside the window is a normal slow load; once the message appears the window is over — check the backend log's listening line, bind to 0.0.0.0, fix the health path, or raise `--startup-timeout` (keep `healthCheckTimeout` at least that long) |
